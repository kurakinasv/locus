...
import { Input, Button, Spacing, PhotoUpload, ThematicBlock } from 'components';
...
import UserStub from 'img/icons/user.svg?react';
      ...
      >
        {({ errors, touched, getFieldProps, isSubmitting, dirty }) => (
          <Form className={s.form}>
            <Field name="photo">
              {({ field, form }: FormikRenderProps<File, FieldValues>) => (
                <PhotoUpload
                  ...
                  stub={<UserStub className={s.stub} />}
            ...
            <Spacing size={3} />
            {fieldsConfig.map(({ name, disabled, ...rest }) => (
